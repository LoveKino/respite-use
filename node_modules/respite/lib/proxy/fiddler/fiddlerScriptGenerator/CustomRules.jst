...
class Handlers {
    ...
    */
    ...
    function OnBeforeRequest(oSession: Session) {
        ...
        //  add-by ddchen
        if ("{{hostnamefilters}}") {
            var acceptTypeReg = /^text\/html.*$/;
            var httpTypeReg = /^http:.*$/;
            var shouldTransmit = acceptTypeReg.test(oSession.oRequest.headers["accept"]) &&
                !oSession.oRequest.headers.Exists("fiddlerpass") &&
                httpTypeReg.test(oSession.fullUrl)
            if (shouldTransmit) {
                oSession.oRequest.headers.Add("fiddlerhostname", oSession.hostname);
                oSession.oRequest.headers.Add("fiddlerport", oSession.port);
                oSession.hostname = "127.0.0.1";
                oSession.port = "{{appPort}}";
            }
        }
        ...
        if (oSession) {
            ...
            sCookie = sCookie + ";from_fiddler=1";
